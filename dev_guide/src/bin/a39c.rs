...
fn main() {
    ...
    s.send(LightMsg::On).unwrap();
    s.send(LightMsg::ChangeColor(255, 0, 0)).unwrap();
    s.send(LightMsg::ChangeColor(0, 128, 0)).unwrap();
    s.send(LightMsg::ChangeColor(0, 0, 255)).unwrap();
    s.send(LightMsg::Off).unwrap();
